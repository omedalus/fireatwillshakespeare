...
Every turn, the player issues a natural-language instruction over an open channel. That instruction is heard by both an allied artillery unit and an adversary. The ally must decide whether and where to fire based solely on the instruction. The enemy hears the same words and may attempt countermeasures, or even spoof messages to trick the ally into hitting hostages. The enemy can also deploy Chaff to shield a single square from artillery for one turn.
